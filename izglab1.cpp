...
void ImageTransform::grayscale()
{
    ...
    for (uint32_t y = 0; y < cfg->h; y++)
    {
        ...
        {
            auto p = getPixel(x, y);
            auto grayValue = std::round(0.299 * p.r + 0.587 * p.g + 0.114 * p.b);

            setPixel(x, y, RGB(grayValue));
        }
    }
}
...
void ImageTransform::threshold()
{
    ...
    for (uint32_t y = 0; y < cfg->h; y++)
    {
        for (uint32_t x = 0; x < cfg->w; x++)
        {
            auto p = getPixel(x, y);
            if (p.r > th) {
                setPixel(x, y, RGB(COLOR_WHITE));
            }
            else {
                setPixel(x, y, RGB(COLOR_BLACK));
            }
...
void ImageTransform::randomModulation()
{
    ...
    for (uint32_t y = 0; y < cfg->h; y++)
    {
        ...
        {
            auto p = getPixel(x, y);
            int randomValue = getRandomFromRange(-(p.r), 255 - (p.r));

            if ((p.r - randomValue) > th) {
                setPixel(x, y, RGB(COLOR_WHITE));
            }
            else {
                setPixel(x, y, RGB(COLOR_BLACK));
...
void ImageTransform::errorDistribution()
{
    ...
    for (uint32_t y = 0; y < cfg->h; y++)
    {
        for (uint32_t x = 0; x < cfg->w; x++)
        {
            ...
            updatePixelWithError(x+1, y, errDist*3/8);
            updatePixelWithError(x, y+1, errDist*3/8);
            updatePixelWithError(x+1, y+1, errDist*2/8);
        }
    }
}
...
void ImageTransform::toneDependentErrorDistribution()
{
    ...
    for (uint32_t y = 0; y < cfg->h; y++)
    {
        for (uint32_t x = 0; x < cfg->w; x++)
        {
            ...
            if (errDist > 40 || errDist < 215) {
                updatePixelWithError(x + 1, y, errDist*7/16);
                updatePixelWithError(x - 1, y + 1, errDist*3/16);
                updatePixelWithError(x, y+1, errDist * 5/16);
                updatePixelWithError(x + 1, y+1, errDist * 1/16);
            }
            else {
                updatePixelWithError(x + 1, y, errDist * 7 / 24);
                updatePixelWithError(x - 1, y + 1, errDist * 3 / 24);
                updatePixelWithError(x, y + 1, errDist * 5 / 24);
                updatePixelWithError(x + 1, y + 1, errDist * 1 / 24);

                updatePixelWithError(x + 2, y, errDist * 2 / 24);
                updatePixelWithError(x -2 , y+1, errDist *  1/ 24);
                updatePixelWithError(x + 2, y + 1, errDist * 1 / 48);
                updatePixelWithError(x - 2, y +2, errDist * 1 / 24);
                updatePixelWithError(x - 1, y + 2, errDist * 1 / 24);
                updatePixelWithError(x , y + 2, errDist * 2 / 24);
                updatePixelWithError(x + 1, y + 2, errDist * 1 / 48);
            }
